...
@push('styles')
<style>
    .invoice-preview-frame { background: #fff; border: 1px solid #ddd; border-radius: 8px; padding: 0; color: #111; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
    .invoice-preview-content { padding: 30px; min-height: 400px; }
    .template-switch { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 16px; }
    .tpl-switch-btn { padding: 8px 16px; border-radius: 8px; border: 2px solid var(--border-color); background: var(--bg-input); color: var(--text-secondary); cursor: pointer; font-size: 0.82rem; font-weight: 600; transition: all 0.2s ease; text-decoration: none; }
    .tpl-switch-btn:hover { border-color: #667eea; color: var(--text-primary); text-decoration: none; }
    .tpl-switch-btn.active { background: linear-gradient(135deg,#667eea,#764ba2); color: #fff; border-color: #667eea; }
    .action-bar { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 16px; }
    .invoice-meta-card { border-left: 4px solid #667eea; }
    .meta-row { display: flex; justify-content: space-between; padding: 6px 0; border-bottom: 1px solid var(--border-color); }
    ...
    <div class="col-lg-8">
        ...
        <div class="card mb-3">
            <div class="card-body" style="padding:12px 16px;">
                <small class="text-muted d-block mb-2" style="text-transform:uppercase;letter-spacing:0.5px;font-size:0.68rem;font-weight:600;">Switch Template</small>
        ...
        <div class="action-bar">
            @if($template)
            <a href="{{ route('invoices.print', ['invoice' => $invoice->id, 'template' => $template->id]) }}" class="btn btn-primary" target="_blank">
    ...
    <div class="col-lg-4">
        <div class="card invoice-meta-card mb-3">
            <div class="card-header" style="padding:12px 16px;">
                <h6 class="mb-0" style="font-weight:700;"><i class="fas fa-info-circle mr-2" style="color:#667eea;"></i>Invoice Details</h6>
            </div>
            <div class="card-body" style="padding:12px 16px;">
                <div class="meta-row">
        ...
        <div class="card mb-3">
            <div class="card-header" style="padding:12px 16px;">
                <h6 class="mb-0" style="font-weight:700;"><i class="fas fa-calculator mr-2" style="color:#00b894;"></i>Totals</h6>
            </div>
            <div class="card-body" style="padding:12px 16px;">
                <div class="meta-row">
                ...
                @endif
                <div class="meta-row" style="background:rgba(0,184,148,0.1);padding:10px;border-radius:6px;margin-top:8px;">
                    <span class="meta-label" style="font-weight:700;color:var(--text-primary);">Net Amount</span>
                    <span class="meta-value" style="font-size:1.1rem;color:#00b894;">{{ number_format($invoice->net_amount, 2) }}</span>
                </div>
                ...
                <div class="meta-row">
                    ...
                    <span class="meta-value" style="color:#ff7675;">{{ number_format($invoice->balance_amount, 2) }}</span>
                </div>
                @endif
            </div>
        </div>
...
function downloadPdf() {
    ...
    @if($template)
    window.open('{{ route("invoices.print", ["invoice" => $invoice->id, "template" => $template->id]) }}', '_blank');
    @endif
}
</script>
@endpush
