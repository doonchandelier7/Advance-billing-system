...
@section('header', 'New Purchase Return')
...
<style>
    ...
    .party-highlight { border-color: rgba(243,156,18,0.45); background: rgba(243,156,18,0.08); }
...
<form method="POST" action="{{ route('modules.transactions.purchase-returns.store') }}" id="prForm">@csrf
    ...
    <div class="card-body">
        <div class="form-section">
            <div class="row">
                ...
                <div class="col-md-3 form-group"><label>Vendor <span class="text-danger">*</span></label><select name="vendor_id" class="form-control form-control-sm" id="pr-vendor" required><option value="">-- Select --</option>@foreach($vendors as $v)<option value="{{ $v->id }}" data-name="{{ $v->name }}" data-city="{{ $v->city }}" data-district="{{ $v->district }}" data-state="{{ $v->state }}" data-gstin="{{ $v->gstin }}" @selected((string) old('vendor_id') === (string) $v->id)>{{ $v->name }}</option>@endforeach</select></div>
            </div>
        </div>
        <div class="form-section party-highlight">
            <div class="row">
                <div class="col-md-3 form-group"><label>Party Name</label><input type="text" name="party_name" class="form-control form-control-sm" id="pr-party-name" value="{{ old('party_name') }}"></div>
                <div class="col-md-2 form-group"><label>City</label><input type="text" name="city" class="form-control form-control-sm" id="pr-city" value="{{ old('city') }}"></div>
                <div class="col-md-2 form-group"><label>District</label><input type="text" class="form-control form-control-sm" id="pr-district"></div>
                <div class="col-md-2 form-group"><label>State</label><input type="text" name="state" class="form-control form-control-sm" id="pr-state" value="{{ old('state') }}"></div>
                <div class="col-md-3 form-group"><label>GSTIN</label><input type="text" name="gstin" class="form-control form-control-sm" id="pr-gstin" value="{{ old('gstin') }}"></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
...
<div class="card mb-4">
    <div class="card-header"><h5 class="mb-0" style="font-weight:600;"><i class="fas fa-boxes mr-2" style="color:#f39c12;"></i>Return Items</h5></div>
    <div class="card-body">
        <div class="form-section" style="background:rgba(243,156,18,0.05);">
            <div class="row align-items-end">
                ...
                <div class="col-md-1 form-group"><label>&nbsp;</label><button type="button" class="btn btn-warning btn-sm btn-block" onclick="addRow()" style="font-weight:600; color:#fff;"><i class="fas fa-plus"></i> ADD</button></div>
            </div>
        </div>
        <div class="items-scroll mt-3">
            <table class="table table-hover items-table mb-0" id="items-table">
                ...
                <tbody></tbody>
                <tfoot>
                    <tr style="font-weight:700; background:rgba(243,156,18,0.08);">
        ...
        <div class="d-flex flex-wrap align-items-center mt-3" style="gap:10px;">
            ...
            <span class="summary-pill" style="background:rgba(243,156,18,0.2); color:#f39c12;" id="total-net">Net: 0.00</span>
        </div>
...
<div class="d-flex mb-4" style="gap:12px;">
    <a href="{{ route('modules.transactions') }}#purchase-returns" class="btn btn-secondary"><i class="fas fa-arrow-left mr-1"></i> BACK</a>
    <button type="submit" name="after_action" value="generate" class="btn" style="background:linear-gradient(135deg,#f39c12,#e67e22); color:#fff; padding:10px 22px; font-weight:600; border:0; border-radius:8px;"><i class="fas fa-file-alt mr-2"></i>SAVE &amp; GENERATE</button>
    ...
    <a href="{{ route('modules.transactions') }}#purchase-returns" class="btn btn-secondary"><i class="fas fa-times mr-1"></i> CANCEL</a>
</div>
...
<script>
var itemCounter = 0;
document.getElementById('pr-vendor').addEventListener('change', function(){ var o=this.options[this.selectedIndex]; document.getElementById('pr-party-name').value=o.dataset.name||''; document.getElementById('pr-city').value=o.dataset.city||''; document.getElementById('pr-district').value=o.dataset.district||''; document.getElementById('pr-state').value=o.dataset.state||''; document.getElementById('pr-gstin').value=o.dataset.gstin||''; });
...
function addRow() {
    ...
    tr.innerHTML='<td style="font-weight:500;">'+pname+'</td><td>'+q+'</td><td>'+unit.value+'</td><td>'+hsn.value+'</td><td class="text-right">'+r.toFixed(2)+'</td><td class="text-right">'+amount.toFixed(2)+'</td><td class="tax-percent-cell">'+g+'%</td><td class="text-right" style="color:#ff7675;">'+tax.toFixed(2)+'</td><td class="text-right" style="font-weight:700; color:#fdcb6e;">'+net.toFixed(2)+'</td><td><button type="button" class="btn btn-danger btn-sm py-0 px-1" onclick="removeRow(this,'+idx+')"><i class="fas fa-trash" style="font-size:0.7rem;"></i></button></td>';
...
function resetForm(){ document.getElementById('prForm').reset(); document.querySelector('#items-table tbody').innerHTML=''; document.getElementById('hidden-items').innerHTML=''; itemCounter=0; updateTotals(); }
