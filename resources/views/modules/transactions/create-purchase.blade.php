...
<a href="{{ route('modules.transactions') }}#purchases" class="btn btn-secondary btn-sm mb-3"><i class="fas fa-arrow-left mr-1"></i> Back to Transactions</a>

<form method="POST" action="{{ route('modules.transactions.purchases.store') }}" id="purchaseForm">@csrf

<div class="card mb-4" style="background:linear-gradient(135deg,#667eea,#764ba2) !important; border:0 !important;">
    <div class="card-body d-flex justify-content-between align-items-center" style="padding:18px 24px;">
        <h5 class="mb-0" style="color:#fff; font-weight:700;"><i class="fas fa-shopping-cart mr-2"></i>PURCHASE</h5>
    ...
    <div class="card-body">
        <div class="form-section">
            <div class="row">
                ...
                <div class="col-md-3 form-group"><label>Doc. Date</label><input type="date" name="purchase_date" class="form-control form-control-sm" value="{{ date('Y-m-d') }}"></div>
                ...
                <div class="col-md-3 form-group"><label>Vendor <span class="text-danger">*</span></label><select name="vendor_id" class="form-control form-control-sm" id="pur-vendor" required><option value="">-- Select --</option>@foreach($vendors as $v)<option value="{{ $v->id }}" data-name="{{ $v->name }}" data-city="{{ $v->city }}" data-state="{{ $v->state }}" data-gstin="{{ $v->gstin }}">{{ $v->name }}</option>@endforeach</select></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
                <div class="col-md-4 form-group"><label>Party Name</label><input type="text" name="party_name" class="form-control form-control-sm" id="pur-party-name"></div>
                <div class="col-md-2 form-group"><label>City</label><input type="text" name="city" class="form-control form-control-sm" id="pur-city"></div>
                <div class="col-md-3 form-group"><label>State</label><input type="text" name="state" class="form-control form-control-sm" id="pur-state"></div>
                <div class="col-md-3 form-group"><label>GSTIN</label><input type="text" name="gstin" class="form-control form-control-sm" id="pur-gstin"></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
        ...
        <div class="form-section">
            <div class="row">
                ...
                <div class="col-md-3 form-group"><label>Reference / Bill No.</label><input type="text" name="reference" class="form-control form-control-sm" placeholder="Supplier bill no."></div>
            </div>
        </div>
    </div>
</div>
...
<div class="d-flex mb-4" style="gap:12px;">
    <button type="submit" class="btn btn-primary" style="padding:10px 28px; font-weight:600;"><i class="fas fa-save mr-2"></i>SAVE PURCHASE</button>
...
<script>
var itemCounter = 0;
document.getElementById('pur-vendor').addEventListener('change', function(){ var o=this.options[this.selectedIndex]; document.getElementById('pur-party-name').value=o.dataset.name||''; document.getElementById('pur-city').value=o.dataset.city||''; document.getElementById('pur-state').value=o.dataset.state||''; document.getElementById('pur-gstin').value=o.dataset.gstin||''; });
...
function updateTotals(){ var rows=document.querySelectorAll('#items-table tbody tr'), count=rows.length, tA=0, tT=0, tN=0; rows.forEach(function(tr){ var c=tr.querySelectorAll('td'); tA+=parseFloat(c[5].textContent)||0; tT+=parseFloat(c[7].textContent)||0; tN+=parseFloat(c[8].textContent)||0; }); document.getElementById('total-entries').textContent=count; document.getElementById('total-taxable').textContent='Taxable: '+tA.toFixed(2); document.getElementById('total-tax').textContent='GST: '+tT.toFixed(2); document.getElementById('total-net').textContent='Net: '+tN.toFixed(2); }
