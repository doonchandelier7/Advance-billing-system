...
@section('header', 'New Sales Return')
...
<style>
    ...
    .party-highlight { border-color: rgba(232,67,147,0.45); background: rgba(232,67,147,0.08); }
...
<form method="POST" action="{{ route('modules.transactions.sales-returns.store') }}" id="srForm">@csrf
    ...
    <div class="card-body">
        <div class="form-section">
            <div class="row">
                ...
                <div class="col-md-3 form-group doc-date-highlight"><label>Doc. Date</label><input type="date" name="return_date" class="form-control form-control-sm" value="{{ old('return_date', date('Y-m-d')) }}"></div>
                <div class="col-md-3 form-group"><label>Payment Mode</label><select name="payment_mode" class="form-control form-control-sm"><option value="CASH" @selected(old('payment_mode', 'CASH') === 'CASH')>CASH</option><option value="CREDIT" @selected(old('payment_mode') === 'CREDIT')>CREDIT</option></select></div>
                <div class="col-md-3 form-group"><label>Customer <span class="text-danger">*</span></label><select name="customer_id" class="form-control form-control-sm" id="sr-customer" required><option value="">-- Select --</option>@foreach($customers as $c)<option value="{{ $c->id }}" data-name="{{ $c->name }}" data-city="{{ $c->city }}" data-district="{{ $c->district }}" data-state="{{ $c->state }}" data-gstin="{{ $c->gstin }}" @selected((string) old('customer_id') === (string) $c->id)>{{ $c->name }}</option>@endforeach</select></div>
            </div>
        </div>
        <div class="form-section party-highlight">
            <div class="row">
                <div class="col-md-3 form-group"><label>Party Name</label><input type="text" name="party_name" class="form-control form-control-sm" id="sr-party-name" value="{{ old('party_name') }}"></div>
                <div class="col-md-2 form-group"><label>City</label><input type="text" name="city" class="form-control form-control-sm" id="sr-city" value="{{ old('city') }}"></div>
                <div class="col-md-2 form-group"><label>District</label><input type="text" class="form-control form-control-sm" id="sr-district"></div>
                <div class="col-md-2 form-group"><label>State</label><input type="text" name="state" class="form-control form-control-sm" id="sr-state" value="{{ old('state') }}"></div>
                <div class="col-md-3 form-group"><label>GSTIN</label><input type="text" name="gstin" class="form-control form-control-sm" id="sr-gstin" value="{{ old('gstin') }}"></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
        ...
        <div class="form-section">
            <div class="row">
                ...
                <div class="col-md-3 form-group"><label>Reference</label><input type="text" name="reference" class="form-control form-control-sm"></div>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-4">
    <div class="card-header"><h5 class="mb-0" style="font-weight:600;"><i class="fas fa-boxes mr-2" style="color:#e84393;"></i>Return Items</h5></div>
    <div class="card-body">
        <div class="form-section" style="background:rgba(232,67,147,0.05);">
            <div class="row align-items-end">
                ...
                <div class="col-md-1 form-group"><label class="tax-meta-label">Unit</label><input type="text" id="add-unit" class="form-control form-control-sm tax-meta-input" readonly></div>
                <div class="col-md-1 form-group"><label class="tax-meta-label">HSN</label><input type="text" id="add-hsn" class="form-control form-control-sm tax-meta-input" readonly></div>
                ...
                <div class="col-md-1 form-group"><label class="tax-meta-label">Tax %</label><input type="number" id="add-gst" class="form-control form-control-sm tax-meta-input" step="0.01"></div>
                <div class="col-md-1 form-group"><label>&nbsp;</label><button type="button" class="btn btn-sm btn-block" onclick="addRow()" style="background:#e84393; color:#fff; font-weight:600; border:0; border-radius:6px;"><i class="fas fa-plus"></i> ADD</button></div>
            </div>
        </div>
        <div class="items-scroll mt-3">
            <table class="table table-hover items-table mb-0" id="items-table">
                ...
                <tbody></tbody>
                <tfoot>
                    <tr style="font-weight:700; background:rgba(232,67,147,0.08);">
        ...
        <div class="d-flex flex-wrap align-items-center mt-3" style="gap:10px;">
            ...
            <span class="summary-pill" style="background:rgba(0,184,148,0.15); color:#00b894;" id="total-amount">Amount: 0.00</span>
            <span class="summary-pill" style="background:rgba(255,118,117,0.15); color:#ff7675;" id="total-tax">GST: 0.00</span>
            <span class="summary-pill" style="background:rgba(232,67,147,0.2); color:#e84393;" id="total-net">Net: 0.00</span>
        </div>
...
<div class="d-flex mb-4" style="gap:12px;">
    <a href="{{ route('modules.transactions') }}#sales-returns" class="btn btn-secondary"><i class="fas fa-arrow-left mr-1"></i> BACK</a>
    <button type="submit" name="after_action" value="generate" class="btn" style="background:linear-gradient(135deg,#e84393,#fd79a8); color:#fff; padding:10px 22px; font-weight:600; border:0; border-radius:8px;"><i class="fas fa-file-alt mr-2"></i>SAVE &amp; GENERATE</button>
    ...
    <a href="{{ route('modules.transactions') }}#sales-returns" class="btn btn-secondary"><i class="fas fa-times mr-1"></i> CANCEL</a>
</div>
...
<script>
var itemCounter = 0;
document.getElementById('sr-customer').addEventListener('change', function(){ var o=this.options[this.selectedIndex]; document.getElementById('sr-party-name').value=o.dataset.name||''; document.getElementById('sr-city').value=o.dataset.city||''; document.getElementById('sr-district').value=o.dataset.district||''; document.getElementById('sr-state').value=o.dataset.state||''; document.getElementById('sr-gstin').value=o.dataset.gstin||''; });
...
function addRow() {
    ...
    tr.innerHTML='<td style="font-weight:500;">'+pname+'</td><td>'+q+'</td><td>'+unit.value+'</td><td>'+hsn.value+'</td><td class="text-right">'+r.toFixed(2)+'</td><td class="text-right">'+amount.toFixed(2)+'</td><td class="tax-percent-cell">'+g+'%</td><td class="text-right" style="color:#ff7675;">'+tax.toFixed(2)+'</td><td class="text-right" style="font-weight:700; color:#fd79a8;">'+net.toFixed(2)+'</td><td><button type="button" class="btn btn-danger btn-sm py-0 px-1" onclick="removeRow(this,'+idx+')"><i class="fas fa-trash" style="font-size:0.7rem;"></i></button></td>';
...
function resetForm(){ document.getElementById('srForm').reset(); document.querySelector('#items-table tbody').innerHTML=''; document.getElementById('hidden-items').innerHTML=''; itemCounter=0; updateTotals(); }
document.getElementById('srForm').addEventListener('submit', function(e){ if(document.getElementById('hidden-items').children.length===0){ e.preventDefault(); alert('Please add at least one item before saving.'); } });
...
$(function(){
    setTimeout(function(){
        if (typeof $.fn.select2 !== 'undefined') {
            $('#sr-customer').select2({ width: '100%', minimumResultsForSearch: 0, placeholder: 'Search customer...' });
