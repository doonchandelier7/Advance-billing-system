...
<a href="{{ route('modules.transactions') }}#sales-returns" class="btn btn-secondary btn-sm mb-3"><i class="fas fa-arrow-left mr-1"></i> Back to Transactions</a>

<form method="POST" action="{{ route('modules.transactions.sales-returns.store') }}" id="srForm">@csrf

<div class="card mb-4" style="background:linear-gradient(135deg,#e84393,#fd79a8) !important; border:0 !important;">
    <div class="card-body d-flex justify-content-between align-items-center" style="padding:18px 24px;">
        <h5 class="mb-0" style="color:#fff; font-weight:700;"><i class="fas fa-exchange-alt mr-2"></i>SALES RETURN</h5>
...
<div class="card mb-4">
    <div class="card-body">
        <div class="form-section">
            <div class="row">
                <div class="col-md-3 form-group"><label>Doc. Type</label><select name="document_type" class="form-control form-control-sm"><option value="Tax Invoice">TAX INVOICE</option><option value="Bill of Supply">BILL OF SUPPLY</option></select></div>
                <div class="col-md-3 form-group"><label>Doc. Date</label><input type="date" name="return_date" class="form-control form-control-sm" value="{{ date('Y-m-d') }}"></div>
                <div class="col-md-3 form-group"><label>Payment Mode</label><select name="payment_mode" class="form-control form-control-sm"><option value="CASH">CASH</option><option value="CREDIT">CREDIT</option></select></div>
                <div class="col-md-3 form-group"><label>Customer <span class="text-danger">*</span></label><select name="customer_id" class="form-control form-control-sm" id="sr-customer" required><option value="">-- Select --</option>@foreach($customers as $c)<option value="{{ $c->id }}" data-name="{{ $c->name }}" data-city="{{ $c->city }}" data-state="{{ $c->state }}" data-gstin="{{ $c->gstin }}">{{ $c->name }}</option>@endforeach</select></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
                <div class="col-md-4 form-group"><label>Party Name</label><input type="text" name="party_name" class="form-control form-control-sm" id="sr-party-name"></div>
                <div class="col-md-2 form-group"><label>City</label><input type="text" name="city" class="form-control form-control-sm" id="sr-city"></div>
                <div class="col-md-3 form-group"><label>State</label><input type="text" name="state" class="form-control form-control-sm" id="sr-state"></div>
                <div class="col-md-3 form-group"><label>GSTIN</label><input type="text" name="gstin" class="form-control form-control-sm" id="sr-gstin"></div>
            </div>
        </div>
        <div class="form-section">
            <div class="row">
...
<div class="d-flex mb-4" style="gap:12px;">
    <button type="submit" class="btn" style="background:linear-gradient(135deg,#e84393,#fd79a8); color:#fff; padding:10px 28px; font-weight:600; border:0; border-radius:8px;"><i class="fas fa-save mr-2"></i>SAVE RETURN</button>
...
<script>
var itemCounter = 0;
document.getElementById('sr-customer').addEventListener('change', function(){ var o=this.options[this.selectedIndex]; document.getElementById('sr-party-name').value=o.dataset.name||''; document.getElementById('sr-city').value=o.dataset.city||''; document.getElementById('sr-state').value=o.dataset.state||''; document.getElementById('sr-gstin').value=o.dataset.gstin||''; });
...
function addRow() {
    ...
    var tbody=document.querySelector('#items-table tbody'), tr=document.createElement('tr'); tr.setAttribute('data-idx',idx);
    tr.innerHTML='<td style="font-weight:500;">'+pname+'</td><td>'+q+'</td><td>'+unit.value+'</td><td>'+hsn.value+'</td><td class="text-right">'+r.toFixed(2)+'</td><td class="text-right">'+amount.toFixed(2)+'</td><td>'+g+'%</td><td class="text-right" style="color:#ff7675;">'+tax.toFixed(2)+'</td><td class="text-right" style="font-weight:700; color:#fd79a8;">'+net.toFixed(2)+'</td><td><button type="button" class="btn btn-danger btn-sm py-0 px-1" onclick="removeRow(this,'+idx+')"><i class="fas fa-trash" style="font-size:0.7rem;"></i></button></td>';
...
function updateTotals(){ var rows=document.querySelectorAll('#items-table tbody tr'), count=rows.length, tA=0, tT=0, tN=0; rows.forEach(function(tr){ var c=tr.querySelectorAll('td'); tA+=parseFloat(c[5].textContent)||0; tT+=parseFloat(c[7].textContent)||0; tN+=parseFloat(c[8].textContent)||0; }); document.getElementById('total-entries').textContent=count; document.getElementById('total-amount').textContent='Amount: '+tA.toFixed(2); document.getElementById('total-tax').textContent='GST: '+tT.toFixed(2); document.getElementById('total-net').textContent='Net: '+tN.toFixed(2); }
